...
if __name__ == '__main__':
    ...
    processes = {}
    responses = {}
    client = Client(process="heartbeat")
    while True:
        while not client._rx_queue.empty():
            ...
            if len(parts) >= 3:
                ...
                if msgtype == "REGISTER":
                    ...
                    client.send("logger", "info", "HEARTBEAT", f"Registered process {procname} with pid={pid}")
                # 解除メッセージの処理
                elif msgtype == "UNREGISTER":
                    if procname in processes:
                        del processes[procname]
                        del responses[procname]
                        client.send("logger", "info", "HEARTBEAT", f"Unregistered process {procname}")
            elif len(parts) >= 2:
                msgtype = parts[0]
                procname = parts[1]
                if msgtype == "ALIVE":
        ...
        for process in processes.keys():
            ...
            try:
                if not psutil.pid_exists(int(pid)):
                    ...
                    client.send("logger", "error", "HEARTBEAT", f"Process {process} with pid={pid} not found. Unregistering.")
                    # サーバーに対して再起動要求を送信
                    client.send("server", "REBOOT")
